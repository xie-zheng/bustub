...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (func_name == "lower" || func_name == "upper") {
    if (args.size() == 1) {
      ...
      auto expr = std::make_shared<StringExpression>(StringExpression(args.at(0), type));
      return expr;
    }
    throw Exception(fmt::format("func call {} should have only 1 VARCHAR arguments", func_name));
  }
